...
# Automatic script to reset the API database without user confirmation
# Usage: ./auto_reset_api_db.sh [keep_count]

API_URL="http://192.168.5.199:8000/api/onboarding"
KEEP_COUNT=${1:-2}  # Default to keeping 2 environments if not specified

echo "Automatically resetting the API database, keeping only the $KEEP_COUNT most recent environments."
